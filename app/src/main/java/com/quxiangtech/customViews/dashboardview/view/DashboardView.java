...
import com.quxiangtech.customViews.dashboardview.Interpolator.SpringInterpolator;
import com.quxiangtech.customViews.dashboardview.util.PxUtils;
import com.quxiangtech.customViews.dashboardview.util.StringUtil;
...
public class DashboardView extends View {
    ...
    int mOilPercent = -1; // 油量进度条
    private Paint mOilProgressPaint;
    ...
    private void initPaint() {
        ...
        mOilProgressPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mOilProgressPaint.setStyle(Paint.Style.STROKE);
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawOil(canvas);
        drawQoe(canvas);
        drawOilProgress(canvas);
        drawQoeProgress(canvas);
        drawPanelOval(canvas);
...
//        drawText(canvas, percent);
    ...
    private void drawPanelOval(Canvas canvas) {
        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setColor(Color.WHITE);
        paint.setStyle(Paint.Style.FILL);

        RectF oval = new RectF(0 + 39, 0 + 39, getWidth() - 39, getHeight() - 39);
        canvas.drawOval(oval, paint);
    }

    private void drawOil(Canvas canvas) {
        RectF oval = new RectF(0 + 20, 0 + 20, getWidth() - 20, getHeight() - 20);
        canvas.drawRect(oval, paintOutCircle);
        ...
        canvas.save();
        canvas.rotate(-45, oval.centerX(), oval.centerY()); // 旋转至起点 -45degrees
        // 描边
        paint.setStrokeWidth(44);
        paint.setColor(Color.BLACK);
        canvas.drawArc(oval, 0, 45 + 90, false, paint);

        // 底色
        paint.setStrokeWidth(40);
        paint.setColor(Color.parseColor("#FFEDEDEE"));
        canvas.drawArc(oval, 0, 45 + 90, false, paint);

        canvas.restore();
    }

    public void startOilAnim() {
        Log.d(TAG, "startOilAnim: ");
    ...
    private void drawOilProgress(Canvas canvas) {
        Log.d(TAG, "drawOilProgress: " + mOilPercent);
        canvas.save();
        canvas.rotate(-40, getWidth() / 2, getHeight() / 2); // 旋转至起点 -45degrees
        RectF oval = new RectF(0 + 20, 0 + 20, getWidth() - 20, getHeight() - 20);

        float startAngle = 0f;
        float sweepAngle = 13f;
        canvas.drawArc(oval, startAngle, sweepAngle * mOilPercent, false, mOilProgressPaint);
        canvas.restore();
    }

    private void drawQoe(Canvas canvas) {
        canvas.save();
        canvas.rotate(-65, getWidth() / 2, getHeight() / 2); // 旋转至起点 -45degrees
        RectF oval = new RectF(0 + 20, 0 + 20, getWidth() - 20, getHeight() - 20);
        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(40);
...
//        canvas.drawRect(arc, paint);
        canvas.drawArc(oval, 0, -(65 + 90), false, paint);
        canvas.restore();
    }

    private void drawQoeProgress(Canvas canvas) {
        canvas.save();
        canvas.rotate(-65, getWidth() / 2, getHeight() / 2); // 旋转至起点 -45degrees
        RectF oval = new RectF(0 + 20, 0 + 20, getWidth() - 20, getHeight() - 20);
        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
...
//        drawInPoint(canvas);
    ...
    private void drawerNum(Canvas canvas) {
        ...
        paintNum.setColor(Color.parseColor("#FFC4C4C4"));
        paintNum.setStrokeWidth(8);
        for (int i = 0; i < mTikeCount; i++) {
            canvas.save(); //记录画布状态
            canvas.rotate(rAngle * i, 0, 0);
            canvas.drawLine(0, numY + 5, 0, numY + 25, paintNum);//画短刻度线
            canvas.restore();
        }
